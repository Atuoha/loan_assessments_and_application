...
a=$(this).serializeArray();console.log(a);let c;c=null===localStorage.getItem("loans")?[]:JSON.parse(localStorage.getItem("loans"));c.push(a);localStorage.setItem("loans",JSON.stringify(c));swal({title:"Application Submitted",text:"Your loan application has been submitted. Wait for our response.",icon:"success",timer:5E3,closeOnClickOutside:!1});setTimeout(()=>{window.location.replace("index.html")},1E4)});$("#login-form").submit(function(a){a.preventDefault();a=$("#email").val();let c=$("#password").val();
"admin@gmail.com"===a&&"00000"===c?(swal({title:"Access Granted",text:"Your access to the loan applications has been granted. Wait for a redirect.",icon:"success",timer:5E3,closeOnClickOutside:!1}),setTimeout(()=>{$("#login").fadeOut("slow");$(".loan_data").fadeIn("slow")},5200)):(swal({title:"Access Denied",text:"Your access to the loan applications has been denied. Unrecognised credentials.",icon:"error",timer:5E3,closeOnClickOutside:!1}),$("#email").val(""),$("#password").val(""))});const h=()=>
{let a=$("#loan_record");if(null==localStorage.getItem("loans")){var c=[];c=document.createElement("p");c.innerHTML="No Record Found";c.className="lead alert alert-danger p-5 text-center";a.append(c);console.log("No Record Found")}else c=JSON.parse(localStorage.getItem("loans")),c.forEach(b=>{const d=document.createElement("td"),f=document.createElement("td"),k=document.createElement("td"),l=document.createElement("td"),m=document.createElement("td"),n=document.createElement("td"),p=document.createElement("td"),
...
timer:1500,closeOnClickOutside:!1});$("#loan_record").load(location.href+" #loan_record");setTimeout(()=>{h();$("#login").fadeOut("slow");$(".loan_data").fadeIn("slow")},1500)});$(".file_upload").click(function(){$("#inpFile").click()});$(".yes_collatoral").click(function(){this.checked?$(".file_upload").css("display","block"):$(".file_upload").css("display","none")})});